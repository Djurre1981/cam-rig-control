...
const BOOM_HALF_RAD = ((BOOM_RANGE_DEG / 2) * Math.PI) / 180;
const BOOM_MAX_RAD = BOOM_REST_ANGLE + BOOM_HALF_RAD;
...
export function minBoomRadiansForGround(
  ...
  let lo = BOOM_REST_ANGLE - BOOM_HALF_RAD;
  let hi = BOOM_MAX_RAD;
...
export function clampBoomRadians(boom: number, head: HeadPose): number {
  const minBoom = minBoomRadiansForGround(head);
